...
# Toolkit
RUN apt-get install -y mysql-client vim nginx php5-fpm phpmyadmin apg
...
RUN sed -i -e"s/localhost/db/" /usr/share/phpmyadmin/config.inc.php  
RUN sed -i -e"s/$dbserver='';/$dbserver='db';/" /etc/phpmyadmin/config-db.php
